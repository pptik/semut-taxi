...
public class RequestRest extends ConnectionHandler {
    ...
    public void updateOnlineStatus(int state){
        Session session = new Gson().fromJson(preferenceManager.getString(Constants.PREF_SESSION_ID), Session.class);
        RequestParams params = new RequestParams();
        params.put("SessionID", session.getSessionID());
        ...
        Log.i(TAG, params.toString());
        post(Constants.REST_USER_UPDATE_ONLINE_STATUS, params, new JsonHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
                Log.i(TAG, "Sending request");
    ...
    public void checkSession(){
        ...
        Log.i(TAG, params.toString());
        post(Constants.REST_CHECK_SESSION, params, new JsonHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
                Log.i(TAG, "Sending request");
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                super.onSuccess(statusCode, headers, response);
                Log.i(TAG, "Success");
                responseHandler.onSuccessRequest(response.toString(), Constants.REST_CHECK_SESSION);
            }
